...
const CartWidget = () =>
{
    ...
    useEffect(() =>
    {
        ...
        {
            if (event.target === cartWidget || event.keyCode === 27)
            {
                        ...
                        userProductCart.map(({ img, title, price, counter, _id }, i) =>
                        {
                            return (
                                <li key={i} className="cart-widget__product-item">
                                    <Link to={`/product/${ _id }`}
                                        onClick={() => { dispatch(changeDisplayCartWidget(false)); document.body.style.overflow = '' }}
                                        className="cart-widget__product-link">

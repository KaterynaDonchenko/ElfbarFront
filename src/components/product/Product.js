...
const ProductMain = () =>
{
    ...
    useEffect(() =>
    {
        ...
        if (showSelect)
        {
            const selectVariants = document.querySelector('.product__select')

            const hiddenDropdownSelect = event =>
            {
                if (selectVariants && !selectVariants.contains(event.target))
                {
                    onToggleDropdown();
                }
            }
            ...
            window.addEventListener('click', hiddenDropdownSelect);
            window.addEventListener('keydown', handleEscape);

            return () =>
            {
                window.removeEventListener('click', hiddenDropdownSelect);
    ...
    const renderSelect = (arr) =>
    {
        ...
        const variants = arr
            .filter((item) =>
            {
                ...
                {
                    mainVariant = (
                        <div key={item._id} onClick={onToggleDropdown} className="product__select-mainVariant">
            ...
            .map((item) => (
                <div key={item._id} className="product__select-variant">
                    <Link to={`/product/${ item._id }`}>{item.taste}</Link>
                </div>
            ));
